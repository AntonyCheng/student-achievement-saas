package top.sharehome.issuemodule.utils.jwt;
...
public class JwtUtils {
    public final static AES AES = cn.hutool.crypto.SecureUtil.aes(CommonConstant.AES_KEY.getBytes());

    public final static MD5 MD5 = SecureUtil.md5();
     ...
     * @return 密文
     */
    public String md5EnString(String content) {
        try {
            if (ObjectUtils.isEmpty(content)) {
                ThrowUtils.error(RCodeEnum.ENCRYPTED_CONTENT_IS_ABNORMAL);
                return null;
            }
            return MD5.digestHex(content + CommonConstant.MD5_SALT);
        } catch (Exception e) {
            ThrowUtils.error(RCodeEnum.ENCRYPTED_CONTENT_IS_ABNORMAL);
            return null;
        }
